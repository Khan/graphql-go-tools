...
`, `
	...
	t.Run("federated interface key selection only", RunTest(
		`
		type Query {
			user: User
		}
		...
		&plan.SynchronousResponsePlan{
			Response: &resolve.GraphQLResponse{
				Data: &resolve.Object{
					Fetch: &resolve.SingleFetch{
						BufferId:              0,
						Input:                 `{"method":"POST","url":"http://user.service","body":{"query":"{user {pets {__typename ... on Cat {id} ... on Dog {id}}}}"}}`,
						DataSource:            &Source{},
						DataSourceIdentifier:  []byte("graphql_datasource.Source"),
						ProcessResponseConfig: resolve.ProcessResponseConfig{ExtractGraphqlResponse: true},
					},
	...
	t.Run("federated interface with concrete types that have different keys", RunTest(
		`
		type Query {
			user: User
		}
		...
		&plan.SynchronousResponsePlan{
			Response: &resolve.GraphQLResponse{
				Data: &resolve.Object{
					Fetch: &resolve.SingleFetch{
						BufferId:              0,
						Input:                 `{"method":"POST","url":"http://user.service","body":{"query":"{user {username pets {__typename ... on Cat {catId} ... on Dog {dogId}}}}"}}`,
						DataSource:            &Source{},
						DataSourceIdentifier:  []byte("graphql_datasource.Source"),
						ProcessResponseConfig: resolve.ProcessResponseConfig{ExtractGraphqlResponse: true},
					},
										...
										Value: &resolve.Array{
											...
											Item: &resolve.Object{
												Fetch: &resolve.BatchFetch{
													Fetch: &resolve.SingleFetch{
														BufferId: 1,
														Input:    `{"method":"POST","url":"http://pet.service","body":{"query":"query($representations: [_Any!]!){_entities(representations: $representations){__typename ... on Cat {name} ... on Dog {name}}}","variables":{"representations":[{"dogId":$$2$$,"catId":$$1$$,"__typename":$$0$$}]}}}`,
														Variables: resolve.NewVariables(
															&resolve.ObjectVariable{
																Path:     []string{"__typename"},
																Renderer: resolve.NewJSONVariableRenderer(),
			...
			DataSources: []plan.DataSourceConfiguration{
				{
					...
					ChildNodes: []plan.TypeField{
						...
						{
							TypeName:   "Cat",
							FieldNames: []string{"catId"},
						},
						{
							TypeName:   "Dog",
							FieldNames: []string{"dogId"},
						},
					},
					Custom: ConfigJson(Configuration{
						Fetch: FetchConfiguration{
						...
						Federation: FederationConfiguration{
							...
							ServiceSDL: `
								...
								extend type Cat implements Pet @key(fields: "catId") {
									catId: ID! @external
									name: String! @external
								}

								extend type Dog implements Pet @key(fields: "dogId") {
									dogId: ID! @external
									name: String! @external
								}
							`,
				...
				},
				{
					RootNodes: []plan.TypeField{
						{
							TypeName:   "Cat",
							FieldNames: []string{"catId", "name"},
						},
						{
							TypeName:   "Dog",
							FieldNames: []string{"dogId", "name"},
						},
					},
					ChildNodes: []plan.TypeField{
						{
					...
					Custom: ConfigJson(Configuration{
						...
						Federation: FederationConfiguration{
							...
							ServiceSDL: `
								...
								type Cat implements Pet @key(fields: "catId") {
									catId: ID!
									name: String!
								}

								type Dog implements Pet @key(fields: "dogId") {
									dogId: ID!
									name: String!
								}
							`,
			...
			Fields: []plan.FieldConfiguration{
				{
					TypeName:       "Cat",
					FieldName:      "name",
					RequiresFields: []string{"catId"},
				},
				{
					TypeName:       "Dog",
					FieldName:      "name",
					RequiresFields: []string{"dogId"},
				},
			},
		}))
			...
			Response: &resolve.GraphQLResponse{
				Data: &resolve.Object{
					Fetch: &resolve.SingleFetch{
						...
						Input:                 `{"method":"POST","url":"http://user.service","body":{"query":"{user {username pets {__typename ... on Cat {id} ... on Dog {id} ... on Cat {id} ... on Dog {id}}}}"}}`,
						DataSource:            &Source{},
						DataSourceIdentifier:  []byte("graphql_datasource.Source"),
						ProcessResponseConfig: resolve.ProcessResponseConfig{ExtractGraphqlResponse: true},
					},
											...
											Item: &resolve.Object{
												Fetch: &resolve.BatchFetch{
													Fetch: &resolve.SingleFetch{
														...
														// harmless.
														Input: `{"method":"POST","url":"http://pet.service","body":{"query":"query($representations: [_Any!]!){_entities(representations: $representations){__typename ... on Cat {catField} ... on Dog {dogField} ... on Cat {name} ... on Dog {name}}}","variables":{"representations":[{"id":$$1$$,"__typename":$$0$$}]}}}`,
														Variables: resolve.NewVariables(
															&resolve.ObjectVariable{
																Path:     []string{"__typename"},
																Renderer: resolve.NewJSONVariableRenderer(),
											...
											Item: &resolve.Object{
												Fetch: &resolve.BatchFetch{
													Fetch: &resolve.SingleFetch{
														...
														// that were inlined. The additional __typename selections are
														// harmless.
														Input: `{"method":"POST","url":"http://pet.service","body":{"query":"query($representations: [_Any!]!){_entities(representations: $representations){__typename ... on Cat {catField}}}","variables":{"representations":[{"id":$$1$$,"__typename":$$0$$}]}}}`,
														Variables: resolve.NewVariables(
															&resolve.ObjectVariable{
																Path:     []string{"__typename"},
																Renderer: resolve.NewJSONVariableRenderer(),
	...
	t.Run("required field included when in shared selection but not fragment", RunTest(
		`
		type Query {
			user: User
		}
		...
		&plan.SynchronousResponsePlan{
			Response: &resolve.GraphQLResponse{
				Data: &resolve.Object{
					Fetch: &resolve.SingleFetch{
						BufferId:              0,
						Input:                 `{"method":"POST","url":"http://user.service","body":{"query":"{user {pets {__typename ... on Dog {id} ... on Cat {id} ... on Dog {id}}}}"}}`,
						DataSource:            &Source{},
						DataSourceIdentifier:  []byte("graphql_datasource.Source"),
						ProcessResponseConfig: resolve.ProcessResponseConfig{ExtractGraphqlResponse: true},
					},
										...
										Value: &resolve.Array{
											...
											Item: &resolve.Object{
												Fetch: &resolve.BatchFetch{
													Fetch: &resolve.SingleFetch{
														BufferId: 1,
														Input:    `{"method":"POST","url":"http://pet.service","body":{"query":"query($representations: [_Any!]!){_entities(representations: $representations){__typename ... on Dog {name}}}","variables":{"representations":[{"id":$$1$$,"__typename":$$0$$}]}}}`,
														Variables: resolve.NewVariables(
															&resolve.ObjectVariable{
																Path:     []string{"__typename"},
																Renderer: resolve.NewJSONVariableRenderer(),
												...
												Fields: []*resolve.Field{
													...
													{
														// No buffer because "id" comes from the parent fetch.
														Name: []byte("id"),
														Position: resolve.Position{
															Line:   0,
															Column: 0,
													...
													},
													{
														// No buffer because "id" comes from the parent fetch.
														//
														// Note: this field is resolved twice because it's selected twice
														// (once as a required field and once as an explicit selection).
														//
														// TODO: skip identical fields.
														Name: []byte("id"),
														Position: resolve.Position{
															Line:   0,
															Column: 0,
	...
	t.Run("required field included when not selected in sibling fragment", RunTest(
		`
		type Query {
			user: User
		}
