...
func TestExecutionEngineV2_GetCachedPlan(t *testing.T) {
	...
	t.Run("should reuse cached plan", func(t *testing.T) {
		...
		cachedPlan := engine.getCachedPlan(firstInternalExecCtx, nil, &gqlRequest.document, &schema.document, gqlRequest.OperationName, &report)
		...
		cachedPlan = engine.getCachedPlan(secondInternalExecCtx, nil, &gqlRequest.document, &schema.document, gqlRequest.OperationName, &report)
		_, oldestCachedPlan, _ = engine.executionPlanCache.GetOldest()
		assert.False(t, report.HasErrors())
		assert.Equal(t, 1, engine.executionPlanCache.Len())
		assert.Equal(t, cachedPlan, oldestCachedPlan.(*plan.SubscriptionResponsePlan))
	...
	t.Run("should create new plan and cache it", func(t *testing.T) {
		...
		cachedPlan = engine.getCachedPlan(secondInternalExecCtx, nil, &differentGqlRequest.document, &schema.document, differentGqlRequest.OperationName, &report)
		_, oldestCachedPlan, _ = engine.executionPlanCache.GetOldest()
		assert.False(t, report.HasErrors())
